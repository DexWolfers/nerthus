...
OUTER:
	for _, test := range testCases {
		...
		for i, v := range test.system.backends {
			...
			if err := c.handlePreprepare(context.TODO(), &message{
				Code:    MsgPreprepare,
				Msg:     m,
				Address: v0.Address(),
			}, val); err != nil {
			...
			expectedCode := protocol.MsgPrepare
			if test.existingBlock {
				expectedCode = protocol.MsgCommit
			}
...
func TestHandlePreprepareWithLock(t *testing.T) {
	...
	mismatchProposal := makeUnit(common.EmptyAddress, common.Hash{}, common.Hash{}, 10)
