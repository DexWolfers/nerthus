...
//		View: &istanbul.View{
//			Round:    big.NewInt(0),
//			Sequence: big.NewInt(int64(proposal.Number())),
//		},
...
//			//sign:= types.NewSignBytes(validator.Address().Bytes())
//
...
//				CommittedSeal: types.NewSignContent(),
//			}, validator); err != nil {
...
//			}
//			if r0.current.Commits.Size() > r0.valSet.TwoThirdsMajority() {
...
//		for range r0.valSet.List() {
//			for _, seal := range committedSeals {
//				//if bytes.Equal(validator.Address().Bytes(), seal.GetSig()[:common.AddressLength]) {
//				if bytes.Equal(types.NewSignContent().GetSig(), seal.GetSig()) {
//					signedCount++
//					break
//				}
//			}
//		}
//		got := signedCount <= r0.valSet.TwoThirdsMajority()
//		require.False(t, got, "the expected signed count should be larger than %v, but got %v", r0.valSet.TwoThirdsMajority(), signedCount)
//		require.True(t, r0.current.IsHashLocked(), "block should be locked")
//	}
//}
...
func TestVerifyCommit(t *testing.T) {
	...
	peer := istanbul.NewValidator(getPublicKeyAddress(privateKey))
	valSet := newTestValidatorSet(int(istanbul.RoundRobin))
	...
	testCases := []struct {
	...
	}{
		{
			...
			commit: &istanbul.Subject{
				...
				Digest: newTestZeroProposal().Hash(),
			},
	...
	for i, test := range testCases {
		...
		if err := c.verifyCommit(common.EmptyAddress, nil, test.commit, peer); err != nil {
