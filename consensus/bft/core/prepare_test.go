...
func TestHandlePrepare(t *testing.T) {
	...
	proposal := newTestProposal(common.EmptyAddress, common.Hash{}, common.Hash{})
...
OUTER:
	for mi, test := range testCases {
		...
		for i, v := range test.system.backends {
			...
			if err := r0.handlePrepare(context.TODO(), &message{
				Code:    MsgPrepare,
				Msg:     m,
				Address: validator.Address(),
			}, validator); err != nil {
				if err != test.expectedErr {
					t.Logf("index:%d>%d", mi, i)
		...
		if decodedMsg.Code != protocol.MsgCommit {
			t.Errorf("message code mismatch: have %v, want %v", decodedMsg.Code, protocol.MsgCommit)
		}
...
func TestVerifyPrepare(t *testing.T) {
	...
	//valSet := istanbul.NewSet([]common.Address{peer.Address()}, istanbul.RoundRobin)
	...
	testCases := []struct {
	...
	}{
		{
			...
			prepare: &istanbul.Subject{
				...
				Digest: newTestProposal(common.EmptyAddress, common.Hash{}, common.Hash{}).Hash(),
			},
