...
func newTester(t *testing.T, confOverride func(*node.Config, *nts.Config)) *tester {
	// Create a temporary storage for the node keys and initialize it
	workspace, err := ioutil.TempDir("", "console-tester-")
	if err != nil {
		t.Fatalf("failed to create temporary keystore: %v", err)
	}
	nodeConf := &node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance}
	ethConf := &nts.Config{
		Genesis:     core.DevGenesisUnit(),
		MainAccount: common.ForceDecodeAddress(testAddress),
	}
	if confOverride != nil {
		confOverride(nodeConf, ethConf)
	}
	// Create a networkless protocol stack and start an Nerthus service within
	stack, err := node.New(nodeConf)
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}

	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return nts.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register Nerthus protocol: %v", err)
	}
	...
	var nerthus *nts.Nerthus
	stack.Service(&nerthus)
...
func TestWelcome(t *testing.T) {
	...
	if want := fmt.Sprintf("mainaccount: %s", testAddress); !strings.Contains(output, want) {
		t.Fatalf("console output missing mainAccount: have\n%s\nwant also %s", output, want)
	}
...
// Tests that the dev mode include some account for test.
func TestDevMode(t *testing.T) {
	var (
		testAccount   accounts.Account
		testPassWorld = "foobar"
	)
	tester := newTester(t, func(nodeConf *node.Config, config *nts.Config) {
		params.RunMode = "dev"
	})

	defer tester.Close(t)
	tester2 := newTester(t, func(nodeConf *node.Config, config *nts.Config) {
		params.RunMode = "dev"
		oldPath, newPath := filepath.Join(tester.workspace, "keystore"), filepath.Join(nodeConf.DataDir, "keystore")

		//copy keystore to new node datadir
		err := os.MkdirAll(oldPath, 0666)
		if err != nil {
			t.Fatal(err)
		}
		...
		keyStore := keystore.NewKeyStore(newPath, 2, 1)
		if testAccount, err = keyStore.NewAccount(testPassWorld); err != nil {
			t.Fatal(err)
		}

	})
	defer tester2.Close(t)
	tester2.console.Evaluate("personal.unlockAccount('" + testAccount.Address.Hex() + "','" + testPassWorld + "')")

	if got := tester2.output.String(); got != "true\n" {
		t.Fatalf("expect personal.unlockAccount success,got %q", got)
	}
