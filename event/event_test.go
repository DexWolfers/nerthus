...
func TestTypeMux_SubscribeWithSize(t *testing.T) {
	mux := new(TypeMux)
	defer mux.Stop()
	...
	wg.Add(size)
	sub := mux.SubscribeWithSize(testEvent(0), size)
	go func() {
		for i := 0; i < size; i++ {
			if err := mux.Post(testEvent(5)); err != nil {
				t.Errorf("Post returned unexpected error: %v", err)
...
func TestMulSubscribers(t *testing.T) {
	var mux = new(TypeMux)
	s1 := mux.Subscribe(testEvent(0))
	s2 := mux.Subscribe(testEvent(0), "")

	go mux.Post(testEvent(1900))
	go mux.Post("Hello Word")
