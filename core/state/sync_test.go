...
func makeTestState() (Database, *ntsdb.MemDatabase, common.Hash, []*testAccount) {
	// Create an empty state
	mem, _ := ntsdb.NewMemDatabase()
	db := NewDatabase(mem)
	state, _ := New(nil, common.Hash{}, db)
...
// account array.
func checkStateAccounts(t *testing.T, db ntsdb.Database, root common.Hash, accounts []*testAccount) {
	// Check root availability and state contents
	state, err := New([]byte{1, 2, 3}, root, NewDatabase(db))
	if err != nil {
		t.Fatalf("failed to create state trie at %x: %v", root, err)
	}
	if err := checkStateConsistency([]byte{1, 2, 3}, db, root); err != nil {
...
func checkTrieConsistency(db ntsdb.Database, root common.Hash) error {
...
func checkStateConsistency(prefix []byte, db ntsdb.Database, root common.Hash) error {
	// Create and iterate a state trie rooted in a sub-node
	if _, err := db.Get(root.Bytes()); err != nil {
		return nil // Consider a non existent state consistent.
	}
	state, err := New(prefix, root, NewDatabase(db))
	if err != nil {
		return err
	}
...
func TestIncompleteStateSync(t *testing.T) {
	...
	for _, node := range added[1:] {
		...
		dstDb.Delete(key)
		if err := checkStateConsistency(nil, dstDb, added[0]); err == nil {
