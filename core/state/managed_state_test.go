...
func create() (*ManagedState, *account) {
	...
	statedb, _ := New([]byte{1, 2, 3}, common.Hash{}, NewDatabase(db))
	ms := ManageState(statedb)
	ms.SetState(addr, statedb)
	statedb.SetNonce(addr, 100)
	ms.accounts[addr] = ms.getAccount(addr)
...
func TestRemoteNonceChange(t *testing.T) {
	...
	account.nonces = append(account.nonces, nn...)
	ms.NewNonce(addr)
	ms.GetState(addr).stateObjects[addr].data.Nonce = 200
	nonce := ms.NewNonce(addr)
	if nonce != 200 {
...
func TestSetNonce(t *testing.T) {
	...
	addr = toAddr([]byte{byte(1)})
	ms.GetState(addr).SetNonce(addr, 1)
