...
func opBalance(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	slot := stack.peek()
	if interpreter.evm.StateDB.Empty(common.BigToAddress(slot)) {
		return nil, vm.ErrNotStrideChain
	}
...
func opGasprice(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	stack.push(interpreter.intPool.get().Set(new(big.Int).SetUint64(interpreter.evm.GasPrice)))
	return nil, nil
}
...
func opBlockhash(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	n := interpreter.intPool.get().Sub(interpreter.evm.UnitNumber, common.Big257)
	if num.Cmp(n) > 0 && num.Cmp(interpreter.evm.UnitNumber) < 0 {
...
func opTimestamp(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	stack.push(math.U256(interpreter.intPool.get().SetInt64(interpreter.evm.Time.Int64() / int64(time.Second))))
	return nil, nil
}

func opNumber(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	stack.push(math.U256(interpreter.intPool.get().Set(interpreter.evm.UnitNumber)))
	return nil, nil
}

func opSCNumber(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	stack.push(math.U256(interpreter.intPool.get().SetUint64(interpreter.evm.SCNumber)))
	return nil, nil
}
...
func opCall(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	if value.Sign() != 0 {
		if !interpreter.cfg.EnableSimulation { //启用模拟器时将不返还Gas
...
func makeLog(size int) executionFunc {
	return func(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
		...
		interpreter.evm.StateDB.AddLog(&types.Log{
			...
			UnitIndex: interpreter.evm.UnitNumber.Uint64(),
		})
