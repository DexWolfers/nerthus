...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	...
	//
	//db, err := ntsdb.NewLDBDatabase(benchDataDir, 128, 1024)
	//if err != nil {
	...
	//	if i%20 == 0 {
	//		db.Close()
	//		db, _ = ntsdb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db ntsdb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*ntsdb.LDBDatabase).NewIterator(nil, nil)
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db ntsdb.Database) {
