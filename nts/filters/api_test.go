...
func TestPublicFilterAPI_NewPendingTransactions(t *testing.T) {
	var (
	...
	startServer := func(addr string) (*rpc.Server, net.Listener) {
		...
		srv.RegisterName("nts", api)
		go http.Serve(l, srv.WebsocketHandler([]string{"*"}))
		return srv, l
	}

	//start
	srv, lis := startServer("127.0.0.1:0")
	defer srv.Stop()

	cli, err := rpc.Dial("ws://" + lis.Addr().String())
	require.NoError(t, err)

	txc := make(chan common.Hash, 3)
	sub, err := cli.NtsSubscribe(context.Background(), txc, "newPendingTransactions")
	require.NoError(t, err)

	go func() {
		//send
		for i := 0; i < 3; i++ {
			txFeed.Send(types.TxPreEvent{true, &types.Transaction{}})
		}
	}()
...
func TestPublicFilterAPI_NewHeads(t *testing.T) {
	var (
		mux        = new(event.TypeMux)
		db, _      = ntsdb.NewMemDatabase()
		txFeed     = new(event.Feed)
	...
	idsc := make(chan types.UnitID, 3)
	sub, err := cli.NtsSubscribe(context.Background(), idsc, "newUnits")
	require.NoError(t, err)

	go func() {
		//send
		for i := 0; i < 3; i++ {
			chainFeed.Send(types.ChainEvent{
				Unit: types.NewUnit(&types.Header{MC: common.StringToAddress("chainA"), Number: uint64(i)}, nil),
			})
		}
	}()
	...
	for {
		...
		case err := <-sub.Err():
			...
			return
		case uid := <-idsc:
			require.Equal(t, int(uid.Height), gots)
			gots++
			if gots == 3 {
				return //ok
			}
