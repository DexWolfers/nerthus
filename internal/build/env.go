...
type Environment struct {
	...
	Buildnum            string
	CommitTime          string // commit datetime
	IsPullRequest       bool
	IsCronJob           bool
}
...
func LocalEnv() Environment {
	env := applyEnvFlags(Environment{Name: "local", Repo: "nerthus/nerthus"})
	...
	if env.CommitTime == "" {
		env.CommitTime = time.Now().Format(time.RFC3339)
	}
