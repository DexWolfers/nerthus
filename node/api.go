...
func (api *PrivateAdminAPI) StartWS(host *string, port *int, allowedOrigins *string, apis *string) (bool, error) {
	...
	if err := api.node.startWS(fmt.Sprintf("%s:%d", *host, *port), api.node.rpcAPIs, modules, origins, true); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PublicAdminAPI) Version() string {
	return api.node.config.Version
}
...
func (api *PublicDebugAPI) RemoveUnit(uhash common.Hash) error {
	type chain interface {
		DagChain() *core.DagChain
	}
	for _, s := range api.node.services {
		if c, ok := s.(chain); ok {
			h := c.DagChain().GetHeaderByHash(uhash)
			if h == nil {
				return errors.New("not found unit by hash")
			}
			return c.DagChain().ClearUnit(uhash, true)
		}
	}
	return errors.New("not found service to celar unit")
}
